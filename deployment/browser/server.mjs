...
async function start() {
  ...
  try {
    ...
    const tokenRequest = fetch('/token')
...
<style>
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  /* LOCAL CHANGE: the audio diagnostics row, meant to be read off the screen of
     a phone where there is no console at all. Empty, it takes up no space. */
  .diag { font-family: var(--font-mono); font-size: 12px; color: var(--text-faint);
  ...
  /* LOCAL CHANGE 5. The home page is for the person receiving the call, not for
     ...
     §6.10 does not happen again). */
  body.simple .diag, body.simple #side, body.simple .meter,
  body.simple #mic, body.simple .pane-head, body.simple .empty { display: none; }
  body.simple main { max-width: 34rem; margin: 0 auto; padding: 2.2rem 1.2rem; }
  body.simple header { display: flex; align-items: baseline; gap: .8rem; flex-wrap: wrap; }
  body.simple header h1 { font-size: 2.4rem; }
  body.simple #status-text { font-size: 1.1rem; }
  body.simple .panes { display: block; }
  body.simple .pane { border: none; background: transparent; }
  body.simple #transcript { font-size: 1.3rem; line-height: 1.6; min-height: 8rem; }
  body.simple .pane-foot { padding: 0; border: none; }
  body.simple #btn { width: 100%; min-height: 5rem; font-size: 1.6rem;
                     border-radius: var(--radius-lg); letter-spacing: normal;
                     text-transform: none; }
  /* "idle", "speaking": console words. Here they are replaced by phrases that
     tell the person whose turn it is. The original text stays in the DOM for the
     /dev page and for screen readers. */
  body.simple #status-text { position: absolute; width: 1px; height: 1px;
                             overflow: hidden; clip-path: inset(50%); }
  body.simple .status::after { font-size: 1.05rem; color: var(--text); }
  body.simple .status.idle::after { content: 'ready when you are'; }
...
// on in silence. It looks harmless and it is not: on 18/09 a server left running
// since 8 September was holding 3000 and serving the OLD agent, while the new
// one started on 3001. Opening localhost:3000 — which is the one you open out of
// habit — you were talking to the zombie, and the browser only said
// "agent_not_found", which is the symptom of something else.
//
// The agent's id is fixed into the page at startup, so an old server is an old
// page: it never updates itself. Better not to start at all than to start on a
// port nobody is going to look at.
const port = Number(process.env.PORT) || 3000
server.on('error', (err) => {
  if (err.code !== 'EADDRINUSE') throw err
  // No "kill them all" advice here, on purpose. On 18/09 exactly that — killing
  // every server.mjs by name — shut down servers running in OTHER open windows.
  // Find the one process on this port, check its folder, stop only that one.
  console.error(`\nERROR: port ${port} is already in use.`)
  console.error(`Often an old server of this project, still serving the PREVIOUS`)
  console.error(`agent. Refusing to start on another port: the page you would open`)
  console.error(`by habit would be the stale one.\n`)
  console.error(`  lsof -nP -iTCP:${port} -sTCP:LISTEN      # which PID holds the port`)
  console.error(`  lsof -a -p <PID> -d cwd                 # which folder it runs in`)
  console.error(`\nIf that folder is this project and nobody is using it: kill <PID>.`)
  console.error(`If it is anything else, leave it alone and use another port:`)
